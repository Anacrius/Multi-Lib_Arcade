...
class Snake
{
    ...
    // Dtor
        ~Snake() { delete [] _table; }
    // Ctor
        Snake(int height, int width) : _height(height), _width(width), _speed(180), _tmpspeed(180), _exit(false), _score(0)
        {
    ...
    // Getter & Setter
        ...
        int   getTmpSpeed(void)  const   {  return _tmpspeed; }
        int   getHeight(void)    const   {  return _height;   }
        int   getWidth(void)     const   {  return _width;    }
        int   getScore(void)     const   {  return _score;    }
        ...
        char *getTable(void)
        {
            std::memset(_table, ' ', _height * _width);

            _table[_body[0].getY() * _width + _body[0].getX()] = 'h';       // head
            for(unsigned int i=1; i<_body.size(); ++i)
                _table[_body[i].getY() * _width + _body[i].getX()] = 'b';   // body
            _table[_apple.getY() * _width+_apple.getX()] = 'a';             // apple
            return _table;
        }
